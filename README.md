...
- `k4.secret` / `k4.secret-pw` — для Ed25519 пар ключей подписи. Формат хранит
  **обе** части пары (приватную и публичную), поэтому мы просим передавать
  `LicensifyPublicKey` даже при работе только с приватным ключом. Это гарантирует
  корректный `k4.sid` идентификатор и восстановление пары без скрытых
  зависимостей. Паролезащищённый вариант подходит для хранения приватных ключей
  в хранилищах.
